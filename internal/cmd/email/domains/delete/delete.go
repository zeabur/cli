...
func runDeleteInteractive(f *cmdutil.Factory, opts Options) error {
	...
	confirmed, err := f.Prompter.Confirm("Are you sure you want to delete this domain?", false)
	if err != nil {
		return err
	}
	if !confirmed {
...
func deleteDomain(f *cmdutil.Factory, opts Options) error {
	s := spinner.New(cmdutil.SpinnerCharSet, cmdutil.SpinnerInterval,
		spinner.WithColor(cmdutil.SpinnerColor),
		spinner.WithSuffix(" Deleting domain..."),
	)
	s.Start()
	err := f.ApiClient.DeleteZSendDomain(context.Background(), opts.id)
	s.Stop()
	if err != nil {
		return err
	}

	f.Log.Infof("Domain deleted successfully")
